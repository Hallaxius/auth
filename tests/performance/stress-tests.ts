	...
	private async makeRequest(): Promise<void> {
		...
		try {
			await new Promise<void>((resolve, reject) => {
				const req = http.get(`${this.config.baseUrl}/`, (res) => {
...
function printStressResults(results: StressTestResults): void {
	...
	console.log(
		`Degradation Point: ${results.degradationPoint} concurrent users`,
	);
	...
	console.log(`\nSystem Resources:`);
	console.log(
		`  Memory: ${results.systemResources.memoryUsageMB.toFixed(2)}MB`,
	);
