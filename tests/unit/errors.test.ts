...
describe("AuthError", () => {
	test("creates error with all fields", () => {
		...
		const error = new AuthError(ErrorCodes.RATE_LIMITED, "test message", {
			statusCode: 400,
			retryable: true,
			retryAfter: 30,
			cause,
	...
	test("defaults retryable to false", () => {
		const error = new AuthError(ErrorCodes.UPSTREAM_ERROR, "msg");
	...
	test("serializes to JSON", () => {
		...
		expect(json).toContain("UPSTREAM_ERROR");
...
describe("isAuthError", () => {
	test("returns true for AuthError instances", () => {
		const error = new AuthError(ErrorCodes.INVALID_TOKEN, "msg");
