...
describe("rateLimit - Full Coverage", () => {
	...
	test("middleware includes all RateLimit headers", async () => {
		const limiter = rateLimit({ maxRequests: 2, windowMs: 60000, storage: new TestRateLimitStorage() });
...
describe("extractIpFromRequest", () => {
	test("ignores x-forwarded-for header (untrusted), uses socket peer", async () => {
		const req = new Request("http://localhost", {
			headers: { "x-forwarded-for": "10.0.0.1" },
		}) as unknown as { socket: { remoteAddress?: string } };
		req.socket = { remoteAddress: "203.0.113.9" };
		expect(await extractIpFromRequest(req)).toBe("203.0.113.9");
	});

	test("does not trust first entry of x-forwarded-for list", async () => {
		const req = new Request("http://localhost", {
			headers: {
	...
	test("ignores x-real-ip header without trusted peer", async () => {
		const req = new Request("http://localhost", {
	...
	test("falls back to socket address", async () => {
		const req = new Request("http://localhost") as unknown as {
	...
	test("handles IPv6 socket peer", async () => {
		const req = new Request("http://localhost", {
...
describe("TestRateLimitStorage - additional coverage", () => {
