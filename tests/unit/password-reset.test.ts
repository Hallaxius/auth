...
import { AuthError, passwordReset } from "../../src/";
...
describe("passwordReset", () => {
	...
	describe("handleForgotPassword", () => {
		...
		test("processes POST request", async () => {
			config = makeConfig({
				...
				forgotPasswordRateLimit: { maxAttempts: 100, windowMs: 60000, storage: new TestBruteForceStorage() },
			});
			manager = passwordReset(config);
			const res = await manager.handleForgotPassword(
	...
	describe("handleResetPassword", () => {
		...
		test("handleResetPassword rate limited", async () => {
			const configLimited = makeConfig({
				forgotPasswordRateLimit: { maxAttempts: 100, windowMs: 60000, storage: new TestBruteForceStorage() },
				resetPasswordRateLimit: { maxAttempts: 1, windowMs: 60000, storage: new TestBruteForceStorage() },
				storage: {
					create: async () => {},
