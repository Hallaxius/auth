...
class MockAuditStorage implements AuditLogStorage {
	...
	async store(entry: AuditEvent): Promise<any> {
		const logEntry = { ...entry, logId: `log_${Date.now()}_${this.counter++}` };
...
describe("audit logger methods", () => {
	...
	test("logDataModification creates audit event", async () => {
		await logger.logDataModification(
			"user123",
			"user_profile",
			"email updated",
		);
	...
	test("logUserAction creates audit event", async () => {
...
describe("audit log query", () => {
	...
	test("queryLogs returns results", async () => {
		await logger.logAuthSuccess("user123");
		await logger.logAuthSuccess("user456");
		await logger.logAuthFailure("user789", "Invalid password");
	...
	test("queryLogs filters by severity", async () => {
		await logger.logAuthSuccess("user123");
		await logger.logBruteForceDetected("192.168.1.1", 10, 60000);
		await logger.logSuspiciousActivity("Suspicious activity");
	...
	test("queryLogs filters by status", async () => {
		await logger.logAuthSuccess("user123");
		await logger.logAuthFailure("user456", "Invalid password");
		await logger.logBruteForceDetected("192.168.1.1", 10, 60000);
	...
	test("queryLogs pagination", async () => {
		for (let i = 0; i < 15; i++) {
			await logger.logAuthSuccess(`user${i}`);
		}
...
describe("audit event structure", () => {
	...
	test("event has correct severity for type", async () => {
		await logger.logAuthSuccess("user123");
		await logger.logBruteForceDetected("192.168.1.1", 10, 60000);
		await logger.logSessionHijackAttempt("user123", "IP mismatch");

		const result = await logger.queryLogs({});

		const authEvent = result.entries.find((e) => e.type === "AUTH_SUCCESS");
		const bruteForceEvent = result.entries.find(
			(e) => e.type === "BRUTE_FORCE_DETECTED",
		);
		const hijackEvent = result.entries.find(
	...
	test("event has correct category for type", async () => {
		await logger.logAuthSuccess("user123");
		await logger.logMfaEnable("user123", "totp");
		await logger.logDataAccess("user123", "profile");
