...
import type { Scope, StoredUser, UserStorage } from "../../src/";
...
function createMockConfig(
	overrides: Partial<{
		...
		csrf: {
			...
			storage: { has: (id: string) => Promise<boolean>; set: (id: string, ttlMs: number) => Promise<void>; setIfAbsent: (id: string, ttlMs: number) => Promise<boolean>; delete: (id: string) => Promise<void>; dispose: () => void };
		};
	}> &
		Record<string, unknown> = {},
) {
	const defaultStorage =
		overrides.storage !== undefined ? overrides.storage : new MockStorage();
		return {
			...
			scopes: (overrides.scopes ?? (["identify", "email"] as Scope[])) as Scope[],
			csrf: {
				enabled: true,
				ttlMs: 300000,
				singleUse: true,
...
describe("discord - OAuth2 flow integration", () => {
	...
	test("handleLogin returns redirect to Discord with state", async () => {
	...
	test("callback without storage still creates session", async () => {
		const configNoStorage = createMockConfig({
			csrf: {
				enabled: true,
				ttlMs: 300000,
				singleUse: true,
