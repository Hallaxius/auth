...
class TestAuditStorage implements AuditLogStorage {
	...
	async query(filter: any) {
		...
		if (filter.type) {
			results = results.filter((e: any) => e.type === filter.type);
		}
		if (filter.userId) {
			results = results.filter((e: any) => e.userId === filter.userId);
		}
		if (filter.severity) {
			results = results.filter((e: any) => e.severity === filter.severity);
		}
		if (filter.status) {
			results = results.filter((e: any) => e.status === filter.status);
		}
		return {
			entries: results,
...
describe("Audit Logging Integration", () => {
	...
	test("logs security violations", async () => {
		...
		const bruteForceEvent = storage.entries.find(
			(e) => e.type === "BRUTE_FORCE_DETECTED",
		);
		expect(bruteForceEvent?.severity).toBe("HIGH");
		expect(bruteForceEvent?.status).toBe("BLOCKED");

		const hijackEvent = storage.entries.find(
			(e) => e.type === "SESSION_HIJACK_ATTEMPT",
		);
	...
	test("logs MFA events", async () => {
		...
		const verifySuccessEvent = storage.entries.find(
			(e) => e.type === "MFA_VERIFY_SUCCESS",
		);
		expect(verifySuccessEvent?.status).toBe("SUCCESS");

		const verifyFailureEvent = storage.entries.find(
			(e) => e.type === "MFA_VERIFY_FAILURE",
		);
	...
	test("logs rate limiting and brute force protection", async () => {
		...
		const rateLimitEvent = storage.entries.find(
	...
	test("logs token revocation", async () => {
		...
		const revokeEvent = storage.entries.find(
			(e) => e.type === "TOKEN_REVOCATION",
		);
	...
	test("logs password reset flow", async () => {
		...
		const requestEvent = storage.entries.find(
			(e) => e.type === "PASSWORD_RESET_REQUEST",
		);
		expect(requestEvent?.context.identifier).toBe("user@example.com");

		const completeEvent = storage.entries.find(
			(e) => e.type === "PASSWORD_RESET_COMPLETE",
		);
	...
	test("sanitizes sensitive data in logs", async () => {
		...
		await logger.logAuthFailure("user@example.com", "Invalid password", {
			context: {
				password: "secret123",
				token: "abc123",
				apiKey: "key_123",
...
describe("Combined Security Hardening", () => {
	...
	test("complete authentication flow with security", async () => {
		...
		expect(headersResult?.headers.get("Content-Security-Policy")).toContain(
			"form-action 'self'",
		);
