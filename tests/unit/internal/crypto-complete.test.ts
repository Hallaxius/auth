...
describe("Crypto AES-GCM - Complete Coverage", () => {
	...
	describe("IV randomness validation", () => {
		...
		test("IV is 16 bytes (128 bits)", async () => {
			...
			const [salt, iv, tag, _ciphertext] = parts;
	...
	describe("performance benchmarks", () => {
		...
		test("key derivation performance", async () => {
			...
			console.log(
				`Key derivation total time (${iterations} iterations): ${totalTime.toFixed(2)}ms`,
			);
		...
		test("large payload performance (100KB)", async () => {
			...
			console.log(
				`Large payload - Encrypt: ${encryptTime.toFixed(2)}ms, Decrypt: ${decryptTime.toFixed(2)}ms`,
			);
			expect(decrypted).toBe(plaintext);
