...
describe("generateState", () => {
	...
	test("user agent hash uses full SHA-256 (64 hex chars) without truncation", async () => {
		const userAgent1 =
		...
		const payload1 = JSON.parse(
			new TextDecoder().decode(fromBase64URL(state1.split(".")[0]!)),
		);
		expect(payload1.userAgentHash).toHaveLength(64);
		...
		const payload2 = JSON.parse(
			new TextDecoder().decode(fromBase64URL(state2.split(".")[0]!)),
		);
		expect(payload2.userAgentHash).toHaveLength(64);
		expect(payload1.userAgentHash).not.toBe(payload2.userAgentHash);
	});
