...
import type { DiscordGuildMember, DiscordUser, UserStorage } from "../../src/";
import { AuthError, DiscordClient, secret } from "../../src/";
import {
	hasAnyRole,
	hasMember,
	hasRole,
...
describe("hasRole", () => {
	beforeEach(() => {
		spyOn(DiscordClient.prototype, "getGuildMember").mockResolvedValue(
	...
	it("returns false on API error", async () => {
		spyOn(DiscordClient.prototype, "getGuildMember").mockRejectedValue(
			new Error("API error"),
		);
		const result = await hasRole(
			"user-id",
...
describe("sync", () => {
	...
	beforeEach(() => {
		mockStorage.findByDiscordId = mock();
		mockStorage.update = mock();
	});
...
describe("join", () => {
	it("adds member successfully without optional params", async () => {
		spyOn(DiscordClient.prototype, "addMember").mockResolvedValue(
			undefined as never,
		);
		await expect(
			join({
	...
	it("throws GuildJoinError on API failure", async () => {
		spyOn(DiscordClient.prototype, "addMember").mockRejectedValue(
			new Error("API error"),
		);
		await expect(
			join({
...
describe("revoke", () => {
	...
	it("revokes token and deletes user on success", async () => {
		...
		spyOn(DiscordClient.prototype, "revokeToken").mockResolvedValue(
			undefined as never,
		);
	...
	it("handles token revocation failure gracefully", async () => {
		...
		mockStorage.delete.mockResolvedValue(undefined);
		spyOn(DiscordClient.prototype, "revokeToken").mockRejectedValue(
