...
export class ComplianceManager {
	...
	async requestDataExport(
		...
		await this.exportStorage.saveRequest(requestId, request);
	...
	async enforceRetentionPolicy(): Promise<{
		...
		for (const policy of this.policies.filter((p) => p.enabled)) {
			...
			for (const userId of users) {
				const categories =
					await this.retentionStorage.getDataCategories(userId);
				...
				switch (policy.action) {
					case "delete":
						await this.retentionStorage.deleteData(
							userId,
						...
						break;
					case "anonymize":
						await this.retentionStorage.anonymizeData(
							userId,
							applicableCategories,
						);
						result.anonymized++;
						break;
					case "archive":
						await this.retentionStorage.archiveData(
							userId,
							applicableCategories,
						);
						result.archived++;
...
export interface DataExportStorage {
	saveRequest(requestId: string, request: DataExportRequest): Promise<void>;
